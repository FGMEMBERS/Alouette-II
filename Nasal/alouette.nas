...
var turbine = props.globals.getNode("sim/model/alouette/turbine-rpm-pct", 1);
var torque_pct = props.globals.getNode("sim/model/alouette/torque-pct", 1);
var stall = props.globals.getNode("rotors/main/stall", 1);
var stall_filtered = props.globals.getNode("rotors/main/stall-filtered", 1);
var stall_filtered2 = props.globals.getNode("rotors/main/stall-filtered2", 1);
var torque_sound_filtered = props.globals.getNode("rotors/gear/torque-sound-filtered", 1);
var torque_sound_filtered2 = props.globals.getNode("rotors/gear/torque-sound-filtered2", 1);
			...
			if (new_state == (2)) {
				...
				if (new_state == (3)) {
					if (rotor_rpm.getValue() > 100) {
						...
						settimer(func { update_state(4) }, 1.5);
...
var update_stall_and_torque_sound = func(dt) {
	...
	stall_filtered2.setDoubleValue(stall_val*10 + 0.006 * ((1 - c-0.5)>0?(1 - c-0.5):0));
...
var rotor_step = props.globals.getNode("sim/model/alouette/rotor-step-deg");
...
var main_loop = func {
	...
	update_torque(dt);
	update_stall_and_torque_sound(dt);
	update_slide();
	update_engine();
...
setlistener("/sim/signals/fdm-initialized", func {
	...
	setlistener("sim/current-view/internal", func {
		var vn = cmdarg().getBoolValue();
		if (vn == 0) {
			is_inside_view.setIntValue(0);
			is_outside_view.setIntValue(1);
		} else {
			is_inside_view.setIntValue(1);
			is_outside_view.setIntValue(0);
		}
	});
